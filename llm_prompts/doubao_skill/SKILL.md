---
name: doubao-extract
description: 识别作业照片中的全部错题并转录为 JSON 数组。当用户发送作业、试卷或练习照片并希望录入错题本时使用;结果只输出 JSON 数组,不附加任何解释文字或代码块标记。
---

请你识别图中的全部题目,仅挑出其中做错或未作答的题,进行转录与解题答疑,结果只输出一个 JSON 数组:
数组里每个元素是一道题。除这个 JSON 数组外,不要输出任何解释文字、前后缀或 ```json 之类的代码块标记。

每个元素严格使用以下 9 个字段(缺值的填空字符串 "" 或空数组 []):
[
 {
  ...
  "suggested_concepts": ["你建议的知识点标签,最多 5 个,每个不超过 20 个字,如 固定搭配/时态混用/去分母;无法判断就填空数组"]
 }
]
...
11. suggested_concepts 只是你的初步建议,宁可少填不要硬凑:标签用通用教学术语(不要抄题干原句),
    同一道题的标签之间不要互相包含。

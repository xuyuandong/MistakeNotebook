...
- 提示词统一在 `server/src/prompts/`(analyze@4/generate@1/verify@1/judge@1/select@1/summarize@1),修改必须递增版本号。
- 模型:`config/models.yaml` 仅 `text_model` 槽位(默认 DeepSeek,openai 协议),普通文本模型即可;密钥在 `.env`,改后重启生效。识题在豆包侧,系统不接视觉模型。
- 测试:shared 13 + server 53 全部通过;`pnpm dev` 打开 http://localhost:5173 即用(免登录)。人工验收清单见 [docs/MANUAL-TEST.md](./docs/MANUAL-TEST.md)。

...
/**
 * 毕业阈值:同一道错题尾部连续答对 N 次(部分正确/答错/放弃都会打断)后不再安排复习。
 * 毕业是确定性派生状态(从作答事实源可重算),不是新的排期状态值(无迁移)。
 * 概念重逢复活后答对一次即可再次毕业(旧连续正确次数仍在尾部)。
 */
export const GRADUATION_STREAK = 3;

/**
 * 概念重逢复活的初始档位(0-based):第 2 档。
 * 毕业题不是"刚错"的新题(第 1 档次日过于急促),也不是维护节奏(末档过疏,
 * 到期时与新错题的关联已冷);取第 2 档(数学默认 10 天 / 语文英语默认 3 天)。
 */
export const REVIVAL_INTERVAL_INDEX = 1;
